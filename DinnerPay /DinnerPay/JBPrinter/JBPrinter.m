 ...
 */
- (void)setAlignment:(JBTextAlignment)alignment
{
 ...
 *  @param fontSize 字号
 */
- (void)setFontSize:(JBFontSize)fontSize
{
...
#pragma mark  文字
- (void)appendText:(NSString *)text alignment:(JBTextAlignment)alignment
{
    [self appendText:text alignment:alignment fontSize:JBFontSizeTitleSmalle];
}

- (void)appendText:(NSString *)text alignment:(JBTextAlignment)alignment fontSize:(JBFontSize)fontSize
{
    // 1.文字对齐方式
    ...
    // 4.换行
    [self appendNewLine];
    if (fontSize != JBFontSizeTitleSmalle) {
        [self appendNewLine];
    }
}
...
{
    [self appendTitle:title value:value fontSize:JBFontSizeTitleSmalle];
}

- (void)appendTitle:(NSString *)title value:(NSString *)value fontSize:(JBFontSize)fontSize
{
    // 1.设置对齐方式
    [self setAlignment:JBTextAlignmentLeft];
    // 2.设置字号
    [self setFontSize:fontSize];
    // 3.设置标题内容
    [self setText:title];
...
{
    [self appendTitle:title value:value valueOffset:offset fontSize:JBFontSizeTitleSmalle];
}

- (void)appendTitle:(NSString *)title value:(NSString *)value valueOffset:(NSInteger)offset fontSize:(JBFontSize)fontSize
{
    // 1.设置对齐方式
    [self setAlignment:JBTextAlignmentLeft];
    // 2.设置字号
...
{
    [self setAlignment:JBTextAlignmentLeft];
    [self setFontSize:JBFontSizeTitleSmalle];
    NSInteger offset = 0;
    if (!isTitle) {
        offset = 10;
    }
...
#pragma mark 图片
- (void)appendImage:(UIImage *)image alignment:(JBTextAlignment)alignment maxWidth:(CGFloat)maxWidth
{
    if (!image) {
        return;
    }
...
{
    [self appendBarCodeWithInfo:info alignment:JBTextAlignmentCenter maxWidth:300];
}

- (void)appendBarCodeWithInfo:(NSString *)info alignment:(JBTextAlignment)alignment maxWidth:(CGFloat)maxWidth
{
...
{
    [self appendQRCodeWithInfo:info size:size alignment:JBTextAlignmentCenter];
}

- (void)appendQRCodeWithInfo:(NSString *)info size:(NSInteger)size alignment:(JBTextAlignment)alignment
{
    [self setAlignment:alignment];
...
{
    [self appendQRCodeWithInfo:info centerImage:nil alignment:JBTextAlignmentCenter maxWidth:250];
}

- (void)appendQRCodeWithInfo:(NSString *)info centerImage:(UIImage *)centerImage alignment:(JBTextAlignment)alignment maxWidth:(CGFloat )maxWidth
{
...
{
    // 1.设置分割线居中
    [self setAlignment:JBTextAlignmentCenter];
    // 2.设置字号
    [self setFontSize:JBFontSizeTitleSmalle];
    // 3.添加分割线
    NSString *line = @"- - - - - - - - - - - - - - - -";
...
- (void)appendFooter:(NSString *)footerInfo
{
    ...
    [self appendText:footerInfo alignment:JBTextAlignmentCenter];
}
